...
export const useCollectInterest = (protocolName: string) => {
    ...
    const collectInterest = async () => {
        try {
            ...
            console.log(`🔄 Starting interest collection for ${protocolName}`);
    ...
    const collectUniswapFees = async (selectedPositions: UniswapPosition[]) => {
        try {
            setError(null);
            setIsConfirmed(false);
            setOperationType('collect');
            ...
            selectedPositions.forEach(position => {
                ...
                const halfFeeUSD = (position.feesUSD || 0) / 2;
                ...
                console.log(`   Total USD fees: $${position.feesUSD || 0}`);
    ...
    const exitPosition = async () => {
        try {
            ...
            console.log(`🔄 Starting position exit for ${protocolName}`);
